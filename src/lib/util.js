...
function flattenRefs (
	...
	if(Array.isArray(definition)) {
		return definition.map((x) => flattenRefs(
			x, seen, depth
		));
	}
	...
	Object.keys(definition).forEach((key) => {
		definition[key] = flattenRefs(
			definition[key], seen, depth + 1
		);
	});
	return definition;
}
...
function makeModel (
	...
	const _types = schemaTypes.convertSchema(
		schema, schema.virtual, conn
	);
	const _refs = {};
	...
	if(schema.virtual === true) {
      ...
      || (freezeTableName && normalizeProp(
      	null, ref, isUnderscored
      ))
      || ref;
		...
		if(typeof model.$class === 'function') {
			_model = model.$class;
			delete model.$class;
			_model.init(_types.props, Object.assign(
				model, schema.options, { sequelize: conn, modelName: ref }
			));
		}
		else {
			_model = conn.define(
				ref, _types.props, Object.assign(model, schema.options)
			);
		}
	...
	// overrides
	Object.assign(
		_model.prototype, model.instanceMethods, _whereOp
	);
	Object.assign(
		_model, model.classMethods, _whereOp
	);
...
function makeFK (
	properties, model, type, sub, db
) {
	Object.keys(model.primaryKeys).forEach((modelPK) => {
		const modelName = normalizeProp(
			null, model.name, model.options.underscored
		);
		const modelProp = normalizeProp(
			model.name, modelPK, model.options.underscored
		);
	...
	Object.keys(models).map((model) => {
		...
		Object.keys(a.refs).forEach((prop) => {
            ...
            && b.target !== _options.$schema.id) {
						...
						// build foreign-keys
						makeFK(
							properties, a.model, b.method, _unique, conn.dialect.name
						);
						makeFK(
							properties, models[b.target].model, b.method, _unique, conn.dialect.name
						);
						...
						// sync foreign-keys
						makeFK(
							_options.$schema.properties, a.model, b.method, _unique, conn.dialect.name
						);
						makeFK(
							_options.$schema.properties, models[b.target].model, b.method, _unique, conn.dialect.name
						);

						makeFK(
							defns[through.model].$schema.properties, a.model, b.method, _unique, conn.dialect.name
						);
						makeFK(
							defns[through.model].$schema.properties, models[b.target].model, b.method, _unique, conn.dialect.name
						);
					}
				}
				...
				if(b.params.foreignKey) {
					makeFK(
						defns[b.target].$schema.properties, a.model, b.method, conn.dialect.name
					);
					makeFK(
						conn.models[b.target].options.$schema.properties, a.model, b.method, conn.dialect.name
					);
				}

				/* istanbul ignore else */
				if(b.method === 'belongsTo') {
					const fixedProp = normalizeProp(
						prop,
					...
					if(fixedAttrs) {
						makeRef(
							defns[a.model.name].$schema.properties, prop, fixedAttrs
						);
						makeRef(
							conn.models[a.model.name].options.$schema.properties, prop, fixedAttrs
						);
					}
				}
			}
		...
		/* istanbul ignore else */
			...
			/* istanbul ignore else */
				if(a.model.rawAttributes[prop].references) {
					...
					const { type } = a.model.rawAttributes[prop];
